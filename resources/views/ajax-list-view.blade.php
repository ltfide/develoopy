<div class="grid grid-cols-1 gap-8 lg:gap-4 my-7 md:grid-cols-2 lg:grid-cols-4">
  @foreach ($data as $row)            
    <div class="border h-96 min-h-full rounded shadow-lg relative">
      ...
      <a class="block text-left px-6 text-xl text-slate-600 font-normal hover:underline" href="/post/{{ $row->slug }}">{{ $row->title }}</a>
      ...
      @endif
      <span class="text-sm text-slate-600 absolute bottom-4 right-[10px]">{{ $row->created_at->diffForHumans() }}</span>
    </div>
  @endforeach
</div>
