<div class="grid grid-cols-1 gap-8 my-7 md:grid-cols-2 lg:grid-cols-3">
  @foreach ($data as $row)            
    <div class="border h-96 min-h-full rounded shadow-lg relative">
      ...
      <a class="inline-block px-6 py-1 bg-green-600 rounded text-white absolute bottom-4 left-0 border shadow hover:bg-green-700" id="btn" href="">{{ $row->category->name }}</a>
    </div>
  @endforeach
</div>
